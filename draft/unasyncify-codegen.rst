...
Code duplication is error-prone, because one has to be very confident that there is no drifting between each variant. We want to have a canonical version!
...
Their strategy was to treat the asynchronous version as canonical, and to generate the code for the synchronous version from the async source.
...
This function is named as the "sync variant", removing the ``a`` (so ``afoo`` becomes ``foo``, ``_afoo`` becomes ``_foo``).

The generated function has the following changes:

* The ``@generate_unasynced_codegen`` decorator is removed
...
This implementation was done in a couple of hours, almost entirely thanks to the existence of ``libCST``. The simplicity of the implementation should be an indicator of the risk.
